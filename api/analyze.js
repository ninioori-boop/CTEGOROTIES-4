...
לדוגמה: "ALIEXPRESS", "ZARA.COM", "בלוברי טי אל וי בעמ", "דור אלון גן יבנה"
❌ אסור לכתוב תיאור כללי כמו "אופנה", "מזון", "דלק", "ביטוח" ב-description!
❌ אסור לתרגם, לקצר, או לשנות את שם בית העסק!
✅ תעתיק את שם בית העסק בדיוק כמו שהוא מופיע בטקסט!

=== חשוב מאוד לגבי סכומים! ===
הסכומים שמופיעים הם הסכומים החודשיים (לא הסכום הכולל של העסקה).
אם מופיע "(תשלום X/Y)" - זו עסקה בתשלומים, הסכום כבר מחולק לחודשי.
פשוט קח את הסכום כמו שהוא - אל תחלק אותו שוב!

=== קטגוריות זמינות (38 קטגוריות) - השתמש רק בהן! ===
⛔ אסור להמציא קטגוריות חדשות! רק הקטגוריות האלה מותרות:
מזון לבית, אוכל בחוץ ובילויים, תחביבים, תקשורת, ביטוח, בריאות, דלק וחניה, תחבצ, ביגוד והנעלה, פארם, תספורת וקוסמטיקה, מתנות, כבישי אגרה, חופשה וטיול, תיקוני רכב, בעלי חיים, ביט ללא מעקב, מזומן ללא מעקב, מיסים, דמי ניהול בניין, עמלות בנק ואשראי, משכנתא, שכר דירה, הוצאות בית, קופת חולים, ביטוח לאומי, חינוך וקייטנות, תרומות, החזר הלוואות, ריהוט והבית, צעצועים, עוזרת בית, סיגריות, ציוד עסקי/משרדי, שונות

=== הוצאות בית כוללת: ===
- ארנונה (עיריות)
- חשמל (חברת החשמל, סופר-פאוור)
- גז (פזגז, סופרגז, אמישראגז)
- מים וביוב (מי אביבים, הגיחון, מי כרמל)

❌ קטגוריות אסורות (אל תשתמש בהן!):
- "מסעדות" → השתמש ב"אוכל בחוץ ובילויים"
- "מזון ומשקאות" → השתמש ב"מזון לבית"
- "מוסדות" → השתמש ב"שונות" או "תחביבים"
- "מזון מהיר" → השתמש ב"אוכל בחוץ ובילויים"
- "תיירות" → השתמש ב"חופשה וטיול"
- "רכב ותחבורה" → השתמש ב"תחבצ"
- "חינוך" → השתמש ב"חינוך וקייטנות"
- "אנרגיה" → השתמש ב"הוצאות בית"
- "פנאי בילוי" → השתמש ב"תחביבים"
- "ארנונה" → השתמש ב"הוצאות בית"
- "חשמל" → השתמש ב"הוצאות בית"
- "גז" → השתמש ב"הוצאות בית"
- "מים וביוב" → השתמש ב"הוצאות בית"

⚠️ אם לא בטוח - שים ב"שונות"!
...
- המקיע הצעיר, שטוקינג גלובל = לרוב בתשלומים - חפש X/Y בעמודות השורה
...
- אם בשורה או בהערות מופיע "ביטול עסקה" או "זיכוי" - חובה להחזיר amount שלילי (אסור להחזיר חיובי)!
